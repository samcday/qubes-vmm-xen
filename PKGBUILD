...
pkgname=("qubes-vm-xen" "qubes-vm-xen-docs")
pkgver=4.14.0
pkgrel=1
...
source=("https://downloads.xenproject.org/release/xen/$pkgver/xen-$pkgver.tar.gz"{,.sig}
        "qubes-builder-rpm-master.tar.gz::https://github.com/qubesos/qubes-builder-rpm/archive/master.tar.gz"
        "xen.spec.in"
        "version"
        "rel"
        "efi-xen.cfg"
...
prepare() {
    qubes-builder-rpm-master/scripts/generate-spec xen.spec.in xen.spec
    patches=$(qubes-builder-rpm-master/tools/spectool xen.spec -P | grep "^Patch" | sed 's/^Patch[0-9]\+: //')

    for patch in ${patches}; do
        cat ../$patch | patch -s -F0 -E -p1 --no-backup-if-mismatch -d xen-$pkgver
    done
